...
class EnvEditor(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        env_example_path = os.path.join(
            os.path.dirname(self.env_file_path), ".env.example"
        )
        ...
        # 2. Si no existe .env, intentar cargar desde ormg/.env.example
            ...
            try:
                ...
                with open(self.env_file_path, "w") as f:
                    f.write(content)
                console.print(
                    f"Archivo [bold]{self.env_file_path}[/bold] creado con variables de [bold]ormg/.env.example[/bold]"
                )
            except Exception as e:
...
def edit_env_variables():
    ...
    try:
        ...
        console.print(
            f"[bold red]Error al editar variables de entorno:[/bold red] {str(e)}"
        )
        return False
    return True
